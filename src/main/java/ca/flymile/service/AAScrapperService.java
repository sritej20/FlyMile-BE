...
import ca.flymile.Model.FlightSlices;
import ca.flymile.Model.Slice;
import ca.flymile.dto.FlightDto;
import ca.flymile.dto.FlightMapper;
...
import static ca.flymile.API.RequestHandler.requestHandler;
...
public class AAScrapperService {
     ...
     */
    public List<List<FlightDto>> getFlightDataList(String origin, String destination, String start, String end, int numPassengers, boolean upperCabin) {
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            ...
            CompletableFuture<List<FlightDto>> future = CompletableFuture.supplyAsync(() -> fetchFlightData(stringDate, origin, destination, numPassengers, upperCabin), pool);
            futures.add(future);
        }
     ...
     *
     */
    private List<FlightDto> fetchFlightData(String date, String origin, String destination, int numPassengers, boolean upperCabin) {
        // Perform HTTP request to obtain flight data in JSON format
        String json = requestHandler(date, origin, destination, numPassengers, upperCabin);
